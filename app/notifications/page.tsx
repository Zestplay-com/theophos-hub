...
export default function Notifications(){
 ...
 useEffect(()=>{const unsub=onAuthStateChanged(auth,async u=>{if(!u){router.replace('/login');return}try{const snap=await getDocs(query(collection(db,'notifications'),where('userId','==',u.uid),limit(50)));const next:any[]=snap.docs.map((d:any)=>({id:d.id,...d.data()})).sort((a:any,b:any)=>{const at=a.createdAt?.toMillis?.()||0;const bt=b.createdAt?.toMillis?.()||0;return bt-at}).slice(0,30);setItems(next)}catch{setItems([])}finally{setLoading(false)}});return()=>unsub()},[router])
